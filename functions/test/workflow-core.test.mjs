...
const queue = (changes = {}) => ({ queueRevision: 7, currentMatchKey: 'M2', nextMatchKey: 'M3', normalCursorMatchKey: 'M2', priorityEntries: [], nextPrioritySequence: 0, ...changes });
...

{
  ...
  assert.deepEqual(selectQueueView(q, busyAssignments, busyWorkflows), { currentMatchKey: 'M2', nextMatchKey: 'R2' });
}

{
  ...
  assert.deepEqual(projectCancel(active, a, editing, 'R1', { draftState: 'rejected' }), projectForceRelease(active, a, editing, 'R1', { draftState: 'rejected' }));
}
...
console.log('workflow-core fixtures passed');
