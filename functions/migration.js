...
async function requireAdmin(request) { if (request.data?.tournamentId !== TOURNAMENT_ID) throw new HttpsError('invalid-argument', 'Unknown tournament.'); if (!request.auth?.uid) throw new HttpsError('unauthenticated', 'Authentication required.'); if (!(await root().collection('admins').doc(request.auth.uid).get()).exists) throw new HttpsError('permission-denied', 'Seeded administrator required.'); if ((await root().get()).data()?.maintenance?.enabled === true) throw new HttpsError('failed-precondition', 'Tournament maintenance is active.'); return request.auth.uid; }
async function assertWritable(tx) { const tournament=await tx.get(root()); if(tournament.data()?.maintenance?.enabled===true) throw new HttpsError('failed-precondition','Tournament maintenance is active.'); }
...
export async function createMigrationManifest(request) {
  ...
  const unresolved=items.filter((item)=>item.classification === 'partial' || item.classification === 'invalid'); const data={manifestId, status:unresolved.length?'blocked':'ready', fixedTeam:request.data?.fixedTeam ?? null, items, unresolvedCount:unresolved.length, createdBy:uid, createdAt:FieldValue.serverTimestamp()}; await db().runTransaction(async(tx)=>{await assertWritable(tx); const current=await tx.get(root().collection('migrationManifests').doc(manifestId)); if(!current.exists)tx.create(current.ref,data);}); return { ...data, createdAt: null };
}
