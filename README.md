...
## 0. 로컬에서 열 때는 꼭 이렇게 (중요)

`admin.html`을 파일 아이콘 더블클릭으로 직접 열면(주소창이 `file://...`로 시작하는 경우) 일부 브라우저(특히 크롬 계열)에서 보안 정책 때문에 페이지 안의 모든 기능이 조용히 동작을 멈추는 경우가 있습니다 — 버튼이 안 눌리고, 에러도 안 보이고, 대시보드는 계속 "불러오는 중"에 머무는 증상이 전형적입니다.

이걸 막으려면 폴더 안의 **`로컬서버_시작.command`** 파일을 더블클릭하세요.
1. 터미널 창이 하나 뜨고, 1초 후 관리자 페이지가 `http://localhost:8765/admin.html` 주소로 자동 열립니다.
2. 대시보드는 관리자 페이지의 "대시보드 열기" 링크를 누르면 같은 방식으로 열립니다.
3. 사용을 마치면 그 터미널 창을 닫으면 서버가 종료됩니다 (다시 쓸 때는 다시 더블클릭).

배포(아래 2번)를 마치고 `https://...web.app` 주소로 접속하는 경우에는 이미 `http(s)://`이므로 이 문제와 무관합니다 — 대회 당일에는 배포된 주소를 쓰는 것을 권장합니다.
