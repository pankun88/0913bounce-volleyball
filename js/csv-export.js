...
import { officialResultFields } from "./bracket.js";
...
function publicOfficialMatch(match) {
  return { ...match, ...officialResultFields(match) };
}
...
export function buildFullResultsCsv({ tournamentName, groups, teams, prelimMatches, finalMatches }) {
  ...
  groups.forEach((g) => {
    ...
    const groupMatches = prelimMatches
      .filter((m) => m.groupId === g.id)
      .map(publicOfficialMatch)
